...
void LCD_Init(void) {
    ...
    GPIO_Initure.Pin = GPIO_PIN_5;            //PB5,±³¹â¿ØÖÆ
...
void LCD_update_framebuffer(uint16_t xs, uint16_t ys, uint16_t width, uint16_t height, uint16_t *color) {
    uint16_t x, y;
    uint16_t str;

    LCD_Set_Window(xs, ys, width, height);
